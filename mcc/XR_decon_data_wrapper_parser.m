...
ip.addParameter('cpuOnlyNodes', true, @(x) islogical(x) || ischar(x));
...
if ischar(cpuOnlyNodes)
    cpuOnlyNodes = strcmp(cpuOnlyNodes,'true');
end
...
XR_decon_data_wrapper(dataPaths,'deconPathstr',deconPathstr,'Overwrite',Overwrite,...
    'ChannelPatterns',ChannelPatterns,'SkewAngle',SkewAngle,'dz',dz,...
    'xyPixelSize',xyPixelSize,'Reverse',Reverse,'ObjectiveScan',ObjectiveScan,...
    'Save16bit',Save16bit,'onlyFirstTP',onlyFirstTP,...
    'parseSettingFile',parseSettingFile,'flipZstack',flipZstack,...
    'Decon',Decon,'RotateAfterDecon',RotateAfterDecon,'cudaDecon',cudaDecon,...
    'cppDecon',cppDecon,'cppDeconPath',cppDeconPath,'loadModules',loadModules,...
    'cudaDeconPath',cudaDeconPath,'OTFGENPath',OTFGENPath,...
    'Background',Background,'dzPSF',dzPSF,'EdgeErosion',EdgeErosion,...
    'ErodeByFTP',ErodeByFTP,'deconRotate',deconRotate,'psfFullpaths',psfFullpaths,...
    'rotatePSF',rotatePSF,'DeconIter',DeconIter,'RLMethod',RLMethod,...
    'wienerAlpha',wienerAlpha,'OTFCumThresh',OTFCumThresh,'skewed',skewed,...
    'fixIter',fixIter,'errThresh',errThresh,'debug',debug,'saveStep',saveStep,...
    'psfGen',psfGen,'GPUJob',GPUJob,'BatchSize',BatchSize,'BlockSize',BlockSize,...
    'largeFile',largeFile,'largeMethod',largeMethod,'zarrFile',zarrFile,...
    'saveZarr',saveZarr,'deconMaskFns',deconMaskFns,'parseCluster',parseCluster,...
    'parseParfor',parseParfor,'jobLogDir',jobLogDir,'cpusPerTask',cpusPerTask,...
    'cpuOnlyNodes',cpuOnlyNodes,'uuid',uuid,'maxTrialNum',maxTrialNum,...
    'unitWaitTime',unitWaitTime,'MatlabLaunchStr',MatlabLaunchStr,'SlurmParam',SlurmParam);
